...
First time: python twitter_personal_poster.py
  → Browser opens, you login manually (with 2FA if needed)
  → Session saved automatically

Next times: python twitter_personal_poster.py
  → Auto-logs in using saved session
  → Posts approved content from AI_Employee_Vault/Approved/X_POST_*.md
"""
...
class TwitterPersonalPoster:
    ...
    def login_to_twitter(self, page, context):
        ...
        try:
            page.goto("https://twitter.com", wait_until="domcontentloaded", timeout=60000)
        ...
        if self._is_logged_in(page):
            print("[LOGIN] ✓ Login verified! Saving session...")
            ...
            try:
                ...
                print(f"[LOGIN] ✓ Session saved to: {self.session_path}")
                print("[LOGIN] Next time you run this, login will be automatic!")
                return True
            except Exception as e:
                print(f"[LOGIN] ⚠️  Warning: Could not save session: {e}")
    ...
    def post_to_profile(self, page, post_content):
        ...
        try:
            ...
            if not clicked:
                print("[POST] ⚠️  Could not find compose area automatically")
            ...
            try:
                ...
                try:
                    ...
                    if not modal.is_visible(timeout=2000):
                        print("[POST] ✓ Post successful via keyboard shortcut!")
                        return True
                except:
            ...
            for i, selector in enumerate(post_button_selectors):
                try:
                    ...
                    if button.count() > 0:
                        ...
                        print(f"[POST] ✓ Successfully clicked Post button!")
                        posted = True
                        break
                    else:
            ...
            for line in lines:
                if line.strip() == '---':
                    if not frontmatter_closed:
                        ...
                        # Second --- separator means end of tweet content
                        break
                    continue
                ...
                if line.startswith('#') or 'Approval' in line or 'Instructions' in line or 'Post Details' in line:
                    skip_next = True
                    continue
        ...
        with sync_playwright() as p:
            ...
            if self.session_path.exists():
                ...
                try:
                    ...
                    print("[SESSION] ✓ Session loaded successfully")
                except Exception as e:
                    print(f"[SESSION] ⚠️  Could not load session: {e}")
    ...
    def run(self):
        ...
        if self.session_path.exists():
            print(f"[INFO] ✓ Session saved - next run will auto-login")
